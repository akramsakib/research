...
Additional commands:
  ...
  strix arena-review ...   Create a local handoff pack for Arena-assisted review
...
def main() -> None:
    ...
    # `strix arena-review …` creates a metadata-only handoff pack for reviewing
    # local workspace source with an Arena assistant. It intentionally does not
    # require an LLM, Docker, or scan setup.
    if len(sys.argv) > 1 and sys.argv[1] == "arena-review":
        from strix.interface.arena_review import run_arena_review

        sys.exit(run_arena_review(sys.argv[2:]))
